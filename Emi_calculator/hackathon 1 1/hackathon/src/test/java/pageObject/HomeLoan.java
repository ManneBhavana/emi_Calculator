...
public class HomeLoan extends BasePage {
		...
		public void loantable() throws IOException, InterruptedException {
		    // Locate headers and rows in the loan table
		    List<WebElement> headers = loan_table.findElements(By.xpath("//tr/th"));
		    List<WebElement> rows = loan_table.findElements(By.xpath("//tr[@class='row no-margin yearlypaymentdetails']"));

		    // Write headers to the Excel file
		    for (int i = 0; i < headers.size(); i++) {
		        ExcelUtilities.write1("LoanTable", 0, i, headers.get(i).getText());
		    }

		    // Write row data to the Excel file
		    for (int j = 0; j < rows.size(); j++) {
		        List<WebElement> cells = rows.get(j).findElements(By.xpath("td"));
		        for (int i = 0; i < cells.size(); i++) {
		            ExcelUtilities.write1("LoanTable", j + 1, i, cells.get(i).getText());
		        }
		        //System.out.println();
		    }
		    System.out.println();
